...
/* ---- dependent systems: parametrize the free variable(s) and pull out sample points ---- */
function particularSolution(s, freeVals){
  let x = Array(s.n).fill(0).map(()=>R(0));
  s.freeIdx.forEach((col,i)=>{ x[col] = freeVals[i]; });
  s.piv.forEach((col,r)=>{
...
function sampleSolutions(s){
  let zeros = s.freeIdx.map(()=>R(0));
  let sol1 = particularSolution(s, zeros);
  let bump = (n)=>s.freeIdx.map((_,i)=> i===0 ? R(n) : R(0));
...
function parametricExpr(s){
  ...
  labels.forEach(({col,letter})=>{ exprs[col]=letter; });
  s.piv.forEach((col,r)=>{
    ...
    labels.forEach(({col:fc,letter})=>{
      ...
      let negative = val(termCoeff)<0, abs = negative?neg(termCoeff):termCoeff;
...
function showResult(rows, s){
  ...
  if(s.type==="unique"){
    ...
    let sampleHtml = samples.map((sol,idx)=>`
...
document.getElementById("copyBtn").onclick = async ()=>{
  ...
  if(last.s.type==="unique"){
    ...
    // fall back to text-rendered parametric expressions with HTML stripped
    let exprs = parametricExpr(last.s).map(e=>e.replace(/<[^>]+>/g,""));
    let samples = sampleSolutions(last.s);
    t = "Infinitely many solutions (consistent, dependent system).\n\n"
      + "General solution:\n" + vs.map((v,i)=>`${v} = ${exprs[i]}`).join("\n")
      + "\n\nTwo sample solutions:\n"
      + samples.map((sol,idx)=>`Solution ${idx+1}: `+vs.map((v,i)=>`${v} = ${str(sol[i])}`).join(", ")).join("\n");
  }
  try{
